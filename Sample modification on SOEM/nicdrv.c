...
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary)
{
   ...
   ifname++;	//dummy operation

   rval = 0;
   
   rval = wiznet_macraw_init(0);
   //if (rval != 0)
      //return 0;
  
  /*
  //for redundancy port
   rval = wiznet_macraw_init(1);
   if (rval != 0)
      return 0;
  */
...
int ecx_outframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   //rval = send(*stack->sock, (*stack->txbuf)[idx], lp, 0);
	rval = wiznet_macraw_send(0, (*stack->txbuf)[idx], lp);
   if (rval == -1)
   {
...
int ecx_outframe_red(ecx_portt *port, int idx)
{
   ...
   if (port->redstate != ECT_RED_NONE)
   {
      ...
      if (wiznet_macraw_send(1, &(port->txbuf2), port->txbuflength2) < 0)
      {
...
static int ecx_recvpkt(ecx_portt *port, int stacknumber)
{
   ...
   bytesrx = recv(*stack->sock, (*stack->tempbuf), lp, 0);
	bytesrx = wiznet_macraw_recv(0, (*stack->tempbuf), lp);
